...
const EventosExternosManagement = () => {
  ...
  const cargarAdhesionesRecibidas = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/eventos-kafka/adhesiones`
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const enviarPublicacion = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/eventos-kafka/publicar`,
        {
          method: "POST",
  ...
  const enviarAdhesion = async () => {
    if (
      !formAdhesion.idOrganizacion ||
      !formAdhesion.idVoluntario ||
      !formAdhesion.nombre ||
    ...
    try {
      ...
      const idOrganizador =
        eventoSeleccionado.idOrganizacion || eventoSeleccionado.createdBy;

      const response = await fetch(
        `${API_BASE_URL}/api/eventos-kafka/adherir-evento/${idOrganizador}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            {loading ? (
              ...
              <div className="text-center py-8 text-gray-500">
                No hay eventos externos disponibles
              </div>
            ) : (
              <Table>
                <TableHeader>
                ...
                {Object.values(adhesionesAgrupadas).map((grupo) => (
                  ...
                  >
                    <h4 className="font-semibold text-gray-900 mb-3">
                      Evento ID: {grupo.idEvento} ({grupo.adhesiones.length}{" "}
                      adhesiones)
                    </h4>
                        ...
                        >
                          <div>
                            ...
                            <p className="text-sm text-gray-600">
                              {adhesion.email}{" "}
                              {adhesion.telefono && `• ${adhesion.telefono}`}
                            </p>
                            <p className="text-xs text-gray-500">
                              Org: {adhesion.idOrganizacion} • Voluntario ID:{" "}
                              {adhesion.idVoluntario}
                            </p>
                          </div>
                        </div>
                      ))}
      ...
      >
        <div className="space-y-4">
          ...
          <Input
            ...
            onChange={(e) =>
              setFormPublicar({ ...formPublicar, fechaHora: e.target.value })
            }
          />
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <Button
              variant="secondary"
              onClick={() => setModalPublicarEvento(false)}
            >
              Cancelar
            </Button>
        ...
        <div className="space-y-4">
          {eventoSeleccionado && (
            <div className="bg-blue-50 p-4 rounded-lg mb-4">
              ...
              <p className="text-sm text-blue-800">
                ...
                {eventoSeleccionado.idOrganizacion}
              </p>
              <p className="text-sm text-blue-800">
                <strong>Fecha:</strong>{" "}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) =>
                setFormAdhesion({ ...formAdhesion, nombre: e.target.value })
              }
            />
            <Input
              label="Apellido *"
              ...
              onChange={(e) =>
                setFormAdhesion({ ...formAdhesion, apellido: e.target.value })
              }
            />
          </div>
          ...
          <Input
            ...
            onChange={(e) =>
              setFormAdhesion({ ...formAdhesion, email: e.target.value })
            }
          />
          ...
          <Input
            ...
            onChange={(e) =>
              setFormAdhesion({ ...formAdhesion, telefono: e.target.value })
            }
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            <Button
              variant="secondary"
              onClick={() => setModalAdherirse(false)}
            >
              Cancelar
            </Button>
